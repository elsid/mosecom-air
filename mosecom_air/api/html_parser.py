...
def select_performed(query):
    ...
    def convert_header(header):
        try:
            ...
            temp['last'] = temp['last'].replace(year=result.year,
            ...
            return result.replace(year=temp['last'].year,
                month=temp['last'].month, day=temp['last'].day)
...
def select_measurements(query, substances, units, performed):
    ...
    (query('tr')
        .filter(lambda i: i >= 2)
        .map(lambda i, v: (PyQuery(v)('td')
            .filter(lambda j: j >= 1))
            .each(lambda j, v: result.append(make_measurement(i, j, v)))))
    return result
